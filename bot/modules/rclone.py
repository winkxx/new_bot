...
def start_rclonecopy(client, message):
    try:
        ...
        sys.stdout.flush()
        run_rclonecopy(onedir=firstdir,twodir=seconddir,client=client,message=message)
...
def start_rclonelsd(client, message):
    try:
        firstdir = message.text.split()[1]
